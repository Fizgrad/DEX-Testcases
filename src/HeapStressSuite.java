...
public class HeapStressSuite {
  ...
  private static void runGcStatus() {
    ...
    try {
      Class<?> amClass = tryLoadClass("android.app.ActivityManager");
      if (amClass == null) {
        amError = "framework class not on bootclasspath";
      } else {
    ...
    NativeHeapInfo nativeHeap = tryGetNativeHeapSize();
    ...
    System.out.println(
        ...
        (nativeHeap.size != null
             ? String.format(Locale.ROOT, "%.1f MB", bytesToMB(nativeHeap.size))
             : "unavailable" +
                   (nativeHeap.error != null ? (" (" + nativeHeap.error + ")")
                                             : "")));
  }

  private static final class NativeHeapInfo {
    final Long size;
    final String error;

    NativeHeapInfo(Long size, String error) {
      this.size = size;
      this.error = error;
    }
  }

  private static NativeHeapInfo tryGetNativeHeapSize() {
    try {
      Class<?> debugClass = tryLoadClass("android.os.Debug");
      if (debugClass == null)
        return new NativeHeapInfo(null, "framework class not on bootclasspath");
      Method m = debugClass.getMethod("getNativeHeapSize");
      return new NativeHeapInfo((Long)m.invoke(null), null);
    } catch (Throwable t) {
      String msg = t.getMessage();
      return new NativeHeapInfo(null, t.getClass().getSimpleName() +
                                      (msg != null ? (": " + msg) : ""));
    }
  }
